...
namespace Minyar {
	public class Program {
        ...
        private static void GenerateClosedItemsets() {
            var path = Path.Combine("..", "..", "..", "data", "all-changed-5-100-ng-mining.txt");
            var miner = new ItTreeMiner(path);
            miner.GenerateClosedItemSets();
            var res = miner.GetMinedItemSets();
            using (var writer = new StreamWriter(Path.Combine("..", "..", "..", "data", "mining", "all-100--ng-changed.json"))) writer.Write(JsonConvert.SerializeObject(res));
        }
